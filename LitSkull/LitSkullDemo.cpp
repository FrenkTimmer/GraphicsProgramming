...
private:
	...
	int mWandVertexOffset;

	UINT mBoxIndexOffset;
	UINT mGridIndexOffset;
	UINT mSphereIndexOffset;
	UINT mCylinderIndexOffset;
	UINT mWandIndexOffset;

	UINT mBoxIndexCount;
	UINT mGridIndexCount;
	UINT mSphereIndexCount;
	UINT mCylinderIndexCount;
	UINT mWandIndexCount;
	...
	mEyePosW(0.0f, 0.0f, 0.0f), mTheta(1.5f*MathHelper::Pi), mPhi(0.1f*MathHelper::Pi), mRadius(15.0f)
{
	...
	XMStoreFloat4x4(&mWandWorld, XMMatrixTranslation(0, 5, 0));
	...
	mGridMat.Ambient = XMFLOAT4(0.48f, 1.0f, 1.0f, 1.0f);
	mGridMat.Diffuse = XMFLOAT4(0.48f, 1.0f, 1.0f, 1.0f);
	...
	mSphereMat.Diffuse = XMFLOAT4(0.2f, 1.0f, 0.0f, 1.0f);
	...
	mSpotLight.Position = XMFLOAT3(0, 5.0f, 0.0f);
	mSpotLight.Ambient = XMFLOAT4(1.0f, 0.0f, 0.0f, 1.0f);
	mSpotLight.Diffuse = XMFLOAT4(1.0f, 0.0f, 0.0f, 1.0f);
	mSpotLight.Specular = XMFLOAT4(1.0f, 0.0f, 0.0f, 1.0f);
	mSpotLight.Att = XMFLOAT3(0.0f, 1.0f, 0.0f);
	mSpotLight.Direction = XMFLOAT3(0, -1.0f, 0);
	mSpotLight.Spot = 10.0f;
	mSpotLight.Range = 50;

	mPointLight.Position = XMFLOAT3(0, 5.0f, 0.0f);
	mPointLight.Ambient = XMFLOAT4(0.3f, 0.3f, 0.3f, 1.0f);
	mPointLight.Diffuse = XMFLOAT4(0.7f, 0.7f, 0.7f, 1.0f);
	mPointLight.Specular = XMFLOAT4(0.7f, 0.7f, 0.7f, 1.0f);
	mPointLight.Att = XMFLOAT3(0.0f, 0.1f, 0.0f);
	mPointLight.Range = 15;
}
...
void LitSkullApp::UpdateScene(float dt)
{
	...
	{
		mPointLight.Position = XMFLOAT3(0, 5.0f, 50.0f);
	}
}
...
void LitSkullApp::DrawScene()
{
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		//Draw the wand
		world = XMLoadFloat4x4(&mWandWorld);
		worldInvTranspose = MathHelper::InverseTranspose(world);
		worldViewProj = world*view*proj;
		...
		activeTech->GetPassByIndex(p)->Apply(0, md3dImmediateContext);
		md3dImmediateContext->DrawIndexed(mWandIndexCount, mWandIndexOffset, mWandVertexOffset);
...
void LitSkullApp::BuildShapeGeometryBuffers()
{
	...
	geoGen.CreateGrid(50.0f, 50.0f, 60, 40, grid);
	geoGen.CreateCylinder(0.2f, 0.2f, 5.0f, 16, 1, wand);
	...
	mWandVertexOffset = mCylinderVertexOffset + cylinder.Vertices.size();
	...
	mWandIndexCount = wand.Indices.size();
	...
	mWandIndexOffset = mCylinderIndexOffset + mCylinderIndexCount;
